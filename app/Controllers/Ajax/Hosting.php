...
class Hosting extends BaseController{
	...
	public function save(){
		try{
			...
			$flag = $this->authentication->check_permission([
				'routes' => 'backend/service/hosting/create'
			]);
			if($flag == false){
	...
	private function store(array $param = []){
		...
		$store['band_width'] = $this->request->getPost('band_width');
		$store['capacity'] = $this->request->getPost('capacity');
		$store['id'] = $this->request->getPost('id');
		$store['price'] = str_replace('.', '', $this->request->getPost('price'));
	...
	private function validation(){
		...
		$errorValidate = [
			'title' => [
				'required' => 'Bạn bắt buộc phải nhập vào tên Hosting!',
			],
		];
		return [
			'validate' => $validate,
