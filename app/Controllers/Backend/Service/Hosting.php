...
class Hosting extends BaseController{
	...
	public function __construct(){
		$this->data = [];		
		$this->data['module'] = 'hosting';
	}

	public function index($page = 1){
		$session = session();
		$flag = $this->authentication->check_permission([
			'routes' => 'backend/service/hosting/index'
		]);
		...
		if($config['total_rows'] > 0){
			$config = pagination_config_bt(['url' => 'backend/service/hosting/index','perpage' => $perpage], $config);
			...
			$this->data['hostingList'] = $this->AutoloadModel->_get_where([
				'select' => 'id, title, price, capacity, band_width, userid_created, userid_updated, publish, (SELECT fullname FROM user WHERE user.id = hosting.userid_created) as creator, created_at, updated_at',
				'table' => $this->data['module'],
				'where' => $where,
				'keyword' => $keyword,
		...
		$this->data['template'] = 'backend/service/hosting/index';
		...
		if($this->request->getMethod() == 'post'){
			...
			if ($this->validate($validate, $errorValidate)){
		 		...
		 		if($insertid > 0){
		 			...
		 			return redirect()->to(BASE_URL.'backend/service/hosting/index');
		 		}
	        }else{
	        	$this->data['validate'] = $this->validator->listErrors();
	        }
		}
		$this->data['template'] = 'backend/service/hosting/store';
	...
	public function update($id = 0){
		...
		$this->data[$this->data['module']] = $this->AutoloadModel->_get_where([
			'select' => 'id, title, band_width, capacity, price',
			'table' => $this->data['module'],
	...
	public function delete($id = 0){
		...
		$this->data['template'] = 'backend/service/hosting/delete';
		return view('backend/dashboard/layout/home', $this->data);
	}
